...
class LSTMTradingAgent(nn.Module):
    ...
    # Zamiast eksploracji, zawsze wybieramy akcję z najwyższą wartością Q
    if long_q_value > short_q_value and long_q_value > 0:
        action = 0  # Long
        ...
        action = 2  # No-Op
        print("Agent wybiera akcję: No-Op")
   ...
   def reward(self, profit_loss, holding_time, market_volatility, new_state, done):
    # Uproszczona funkcja nagrody, której celem jest maksymalizacja zysku
    reward_value = profit_loss * 100  # Zwiększamy mnożnik, aby kara/nagroda była bardziej agresywna

    # Dodanie nagrody za krótki czas trzymania zyskownej pozycji (zysk jest priorytetem)
    if profit_loss > 0 and holding_time <= 5:
        reward_value += profit_loss * 100  # Większa nagroda za szybkie zyski

    if done and profit_loss > 0:
        reward_value += profit_loss * 200  # Dodatkowa nagroda za zamknięcie zyskownej pozycji
    elif done and profit_loss < 0:
        reward_value -= abs(profit_loss) * 50  # Kara za stratną pozycję, mniej agresywna kara niż nagroda za zysk

    self.combined_reward += reward_value
    print(f"Final reward for this action: {reward_value}, combined reward: {self.combined_reward}")

    # Dodanie decyzji do replay memory
    self.replay_memory.append((self.last_state, self.last_action, self.combined_reward, new_state, done))
    if len(self.replay_memory) > self.memory_size:
        self.replay_memory.pop(0)

    # Trening agenta na podstawie nagromadzonych danych
    self.train_agent()

   def train_agent(self):
    if len(self.replay_memory) < self.batch_size:
        return  # Jeśli nie ma wystarczającej ilości danych, nie trenujemy
